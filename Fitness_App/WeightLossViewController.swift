...
class WeightLossViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        titleLabel.text = "Weight Loss Plan"
        ...
        for (index, item) in planItems.enumerated() {
            let videoView = VideoView(title: item.title, description: item.description, videoURL: item.videoURL)
        ...
        NSLayoutConstraint.activate([
            ...
            tabBarViewController.view.bottomAnchor.constraint(equalTo: view.bottomAnchor),
            tabBarViewController.view.heightAnchor.constraint(equalToConstant: 49) // Adjust the height as needed
        ])
    }
}

class WLVideoView: UIView {
